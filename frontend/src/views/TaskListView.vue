  ...
  <div class="space-y-6">
    <BaseCard>
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p class="text-sm uppercase tracking-[0.28em] text-[#4f7df0]">Task catalog</p>
          <h2 class="mt-3 text-4xl font-semibold text-slate-900">Workspace Tasks</h2>
          <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-500">Chọn một workspace để xem toàn bộ task, trạng thái hiện tại và thao tác nhanh ngay trên danh sách.</p>
        </div>
        ...
        <div class="w-full max-w-md lg:w-[360px]">
          <label class="block text-sm font-medium text-slate-600">
            ...
            >
              ...
              <option v-for="workspace in workspaceStore.workspaces" :key="workspace.workspace_id" :value="workspace.workspace_id">
    ...
    <BaseCard v-if="selectedWorkspace">
      <div class="grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-[24px] bg-[#f7f9ff] p-5">
          <p class="text-sm text-slate-400">Visible Tasks</p>
          <p class="mt-2 text-2xl font-semibold text-slate-900">{{ visibleTasks.length }}</p>
        </div>
        <div class="rounded-[24px] bg-[#f7f9ff] p-5">
          <p class="text-sm text-slate-400">In Review</p>
          <p class="mt-2 text-2xl font-semibold text-slate-900">{{ visibleTasks.filter((task) => task.status === 'in_review').length }}</p>
        </div>
      </div>
    </BaseCard>

    <div v-if="selectedWorkspace" class="space-y-4">
      <article v-for="task in visibleTasks" :key="task.id" class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(40,66,120,0.08)]">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-3">
              <TaskStatusBadge :status="task.status" />
              <RouterLink :to="`/workspaces/${selectedWorkspaceId}/tasks/${task.id}`" class="text-2xl font-semibold text-slate-900 hover:text-[#4f7df0]">
                {{ task.title }}
              </RouterLink>
            </div>
            ...
            <div class="mt-4 flex flex-wrap gap-3 text-xs text-slate-400">
              ...
              <span class="rounded-full bg-slate-100 px-3 py-1">Assigned By: {{ task.assign_from_username || task.assign_from || 'Unknown' }}</span>
              <span class="rounded-full bg-slate-100 px-3 py-1">Deadline: {{ formatDate(task.deadline) }}</span>
            </div>
            ...
            <div class="mt-5 rounded-[22px] bg-[#f7f9ff] p-4">
              ...
              <form v-if="canUpload(task)" class="mt-4 space-y-3" @submit.prevent="handleInlineUpload(task.id)">
                ...
                <p v-if="uploadErrors[task.id]" class="rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-600">{{ uploadErrors[task.id] }}</p>
                <div class="flex justify-end">
                  <BaseButton type="submit" :disabled="!selectedFiles[task.id]" :loading="uploadingTaskIds[task.id]">Upload File</BaseButton>
                </div>
              </form>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-2 lg:max-w-[340px] lg:justify-end">
            <BaseButton v-if="canSubmit(task)" size="sm" @click="handleSubmit(task.id)">Submit</BaseButton>
            <BaseButton v-if="canApprove(task)" size="sm" @click="handleApprove(task.id)">Approve</BaseButton>
            <BaseButton v-if="canReject(task)" size="sm" variant="secondary" @click="handleReject(task.id)">Reject</BaseButton>
            <RouterLink :to="`/workspaces/${selectedWorkspaceId}/tasks/${task.id}`" class="inline-flex items-center justify-center rounded-2xl bg-[#eef3ff] px-3 py-2 text-sm font-medium text-[#4f7df0] transition hover:bg-[#e2ebff]">
              Open Detail
            </RouterLink>
          </div>
        </div>
    ...
    <BaseCard v-else class="text-center text-sm text-slate-500">
      Chọn một workspace để xem task.
    </BaseCard>
  </div>
</template>
...
watch(selectedWorkspaceId, async (workspaceId) => {
  ...
  await taskStore.fetchTasks(workspaceId)
})
